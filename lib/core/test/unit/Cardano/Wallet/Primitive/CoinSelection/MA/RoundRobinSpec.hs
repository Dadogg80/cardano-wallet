  ...
  where
    ...
    allSpentOrBurntTokens outputsToCover burntTokens =
      ...
      in
        ...
        -- TokenMap.toFlatList $ requestedTokens
        -- TokenMap.toFlatList $ burntTokens
...
prop_makeChange_success_delta p change =
    let
        ...
        delta = TokenBundle.unsafeSubtract
            (Debug.trace ("totalInputValue-test: " <> pretty (Flat totalInputValue)) totalInputValue)
            totalOutputWithChange
    in
        (delta === TokenBundle.fromCoin (view #requiredCost p))
            & counterexample (counterExampleText delta)

  where
    counterExampleText :: TokenBundle -> String
    counterExampleText delta = unlines
        [ "totalInputValue"
        , pretty (Flat totalInputValue)
        , "totalOutputValue"
        , pretty (Flat totalOutputValue)
        , "totalInputValue - (totalOutputValue + change)"
        , pretty (Flat delta)
        , "required cost"
