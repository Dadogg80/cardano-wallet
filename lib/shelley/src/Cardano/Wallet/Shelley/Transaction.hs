...
mkUnsignedTx era ttl cs md wdrls certs fees = mkShelleyBasedTx
  where
    mkShelleyBasedTx :: Either ErrMkTx (Cardano.TxBody era)
    mkShelleyBasedTx = left toErrMkTx $ Cardano.makeTransactionBody $ Cardano.TxBodyContent
        { Cardano.txIns =
        ...
        , Cardano.txWithdrawals =
            ...
            (map (\(key, coin) -> (key, coin, Cardano.BuildTxWith (Cardano.KeyWitness Cardano.KeyWitnessForStakeAddr))) wdrls)
